...
class TileImage:
    ...
    def on_left_click(self, event=None):
        ...
        if self.createanims.current_tile_image_rectangle is None: #Again, similar approach to PalRectangle and ColorPickerRectangle. Though this time I add a suffix _rectangle to make it clear that we're making a rectangle around the tile image. Wonderful awesome.
            ...
            self.createanims.current_tile_image_inner_rectangle = self.chr_canvas.create_rectangle(x, y, x+13, y+13, width=1, outline="black") #Actually inner, what I meant to say. #Outer, it's going to help for white tiles to be clearly visibly selected as well.
            self.createanims.current_tile_image_outer_rectangle = self.chr_canvas.create_rectangle(x-2, y-2, x+15, y+15, width=1, outline="black") #And now outer, helps a lot too.
        else:
